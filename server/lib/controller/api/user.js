...
const { cache, siteFunc, validatorUtil, service } = require('@utils')
  ...
  try {
    ...
    if (bindType == '1') {
      ...
      if (userInfo.phoneNum) {
        throw new Error(
          res.__('user_action_tips_repeat', {
  ...
  try {
    ...
    if (loginType == '1' || loginType == '2') {
      ...
      if (loginType == '2') {
        ...
        let currentCode = await getCacheValueByKey(
          settings.session_secret + '_sendMessage_login_' + (fields.countryCode + fields.phoneNum)
        )
        if (
          !fields.messageCode ||
          !validator.isNumeric(fields.messageCode.toString()) ||
    ...
    for (const creatorId of creatorIdArr) {
      ...
      renderSuccess(req, res, {
        data: res.__('restful_api_response_success', {
          label: res.__(
            creatorFollowState === 'in' ? 'user_action_tips_add_creator' : 'user_action_tips_unsubscribe_creator'
          )
        })
      })
    }
