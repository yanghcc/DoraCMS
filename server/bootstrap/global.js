...
global.reqJsonData = async (url, params = {}, method = 'get') => {
  ...
  try {
    let targetUrl =
      url.indexOf('manage/') == '0' ? urlConfigs.server_admin_api + '/' + url : urlConfigs.server_api + '/' + url
...
// 封装api返回的数据
global.renderSuccess = (req = {}, res, { code = 200, message = '操作成功', data = {}, type = null } = {}) => {
  let sendData = {
    status: code,
    message: message,
...
global.renderFail = (req, res, { code = 500, message = '操作成功', data = {}, type = null } = {}) => {
...
global.getContentListFields = (type = '') => {
  ...
  if (type == 'normal') {
    files =
      '_id title stitle sImg uAuthor date updateDate discription clickNum roofPlacement type videoImg state dismissReason categories isTop'
  } else if (type == 'simple') {
    files = '_id title stitle sImg stitle date updateDate clickNum roofPlacement type videoImg state dismissReason'
  } else if (type == 'stage1') {
    files =
      '_id title stitle sImg uAuthor date updateDate discription comments clickNum roofPlacement type videoImg state dismissReason categories isTop'
  } else {
    files =
      '_id title stitle sImg uAuthor date discription clickNum roofPlacement type appShowType imageArr videoArr duration simpleComments comments videoImg state dismissReason categories isTop'
  }
