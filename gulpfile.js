...
gulp.task('jsmin', gulp.series('cleanjs', function() {
  return gulp
    .src(doraWhiteNormalJs)
    .pipe(
      babel({
...
gulp.task(
  'pagerjsmin',
  gulp.series('cleanpagerjs', function() {
    return gulp
      .src(doraWhitePagerJs)
      .pipe(
        babel({
          presets: [es2015Preset]
        })
...
gulp.task('uglifyEditorJs', function() {
  gulp.watch(doraWhiteEditor, gulp.series('editorjsmin'))
})

gulp.task('uglifyPagerJs', function() {
  gulp.watch(doraWhitePagerJs, gulp.series('pagerjsmin'))
})

gulp.task('uglifyWhiteJs', function() {
  gulp.watch(doraWhiteNormalJs, gulp.series('jsmin'))
})

gulp.task('uglifyLayerJs', function() {
  gulp.watch(doraLayerSassPath, gulp.series('layerSass'))
})

gulp.task('default', gulp.parallel('uglifyWhiteJs', 'uglifyEditorJs', 'uglifyLayerJs', function() {
  gulp.watch(doraWhiteSassPath, gulp.series('sass'))
}))
