...
const { siteFunc, logUtil } = require('@utils')
const {
...
app.use(
  '/ueditor/ue',
  ueditor(path.join(__dirname, 'public'), (config = qnParams), function(req, res, next) {
...
app.get('/dr-admin', async (req, res, next) => {
...
app.get('*', (req, res) => {
  let Page404 = `
        <div style="text-align:center">
            ...
            <div style="font-size: 15px;color: #878d99;">${res.__('label_page_404')} &nbsp;<a href="/">${res.__(
    'label_backto_index'
  )}</a></div>
        </div>
    `
